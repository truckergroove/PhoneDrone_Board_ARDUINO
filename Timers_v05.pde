void OutputCh(byte ch, int pwm)
{
 ...
 switch(ch)
  {
    case 0:  OCR5B=pwm; break;  //out1 = AILERON
    case 1:  OCR5C=pwm; break;  //out2 = ELEVATOR
    case 2:  OCR1B=pwm; break;  //out3 = RUDD
    case 3:  OCR1C=pwm; break;  //out4 = THROTTLE
    
    //lower cases not used temporaly..
...
int InputCh(byte ch)      //here ch going from 2 till 6
{
  return (PWM_RAW[ch]+600)/2;
}

void Init_PWM1(void)            //OUT3&4
{
  pinMode(11,OUTPUT);  //PB5
  pinMode(12,OUTPUT);  //out3
  pinMode(13,OUTPUT);  //out4
...
void Init_PWM3(void)             //OUT7&8 the pin 4 is not identified on the phonedrone
{
  pinMode(2,OUTPUT);  //out8
  pinMode(3,OUTPUT);  //out7
  pinMode(4,OUTPUT);
...
void Init_PWM5(void)             //OUT1&2
{
  pinMode(44,OUTPUT);  //out2
  pinMode(45,OUTPUT);  //out1
  pinMode(46,OUTPUT);  //PL3 not used in phonedrone
...
void Init_PPM_PWM4(void)          //OUT4&5
{
  pinMode(49, INPUT);
  pinMode(7,OUTPUT);
  pinMode(8,OUTPUT);
